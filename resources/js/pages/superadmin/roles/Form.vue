...
type RoleFormValues = { id?: number; name: string; description: string | null };
    ...
    <Form :action="submitAction.url" :method="submitAction.method" class="flex flex-col gap-6"
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-1">
            <div class="grid gap-2">
                ...
                <Input id="name" name="name" :default-value="role?.name" required placeholder="e.g. Admin" />
                <InputError :message="errors.name" />
            </div>

            <div class="grid gap-2">
                <Label for="description">Description</Label>
                <Textarea id="description" name="description" :default-value="role?.description ?? ''"
